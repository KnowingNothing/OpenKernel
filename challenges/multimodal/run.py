...
def run_train_test_with_profiler(use_custom_kernel: float = False):  
    ...
    try: 
        ...
        with torch.profiler.profile( 
            ...
            # =========================================================================
            # Gain the average time of effective operation
            # =========================================================================

            print("\n" + "="*60)
            print("📊 Final Profiler Analysis for 'ProfilerStep*'")
            print("="*60)

            # 1. use the key_averages() to get the aggregated data of 'ProfilerStep*' events
            key_averages_events = prof.key_averages()

            # 2. find the event named "ProfilerStep*" in the results
            profiler_step_summary = None
            for event in key_averages_events:
                if event.key == "ProfilerStep*":
                    profiler_step_summary = event
                    break

            # 3. If the event is found, extract the pre-computed average time
            if profiler_step_summary:
                # For schedule-generated summary events:
                # - profiler_step_summary.count             is the number of active_steps (e.g., 3)
                # - profiler_step_summary.cuda_time_total   is the total GPU time for all active steps
                # - profiler_step_summary.cuda_time         is the average GPU time per step computed by PyTorch

                # We directly use the .cuda_time attribute, which is in microseconds (us), and then convert it to milliseconds (ms)
                average_time_ms = profiler_step_summary.cuda_time / 1000

                print(f"Found the summary event: '{profiler_step_summary.key}'")
                print(f"Number of active steps recorded in summary: {profiler_step_summary.count}")
                print("------------------------------------------------------------")
                print(f"Correct Average GPU Time per Active Step: {average_time_ms:.3f} ms")
                print("="*60)
            else:
                print("⚠️ Error: Could not find the summary event 'ProfilerStep*'.")
                print("   Please ensure that prof.step() was called inside the profiler loop.")
        ...
        print(f"  - Output Tensor Mean: {output_float.mean().item():.4f}") 
...
def run_inference_test_with_profiler(use_custom_kernel: float = False): 
    ...
    try: 
        ...
        with torch.profiler.profile( 
            ...
            # 1. use the key_averages() to get the aggregated data of 'ProfilerStep*' events
            key_averages_events = prof_prefill.key_averages()
            ...
            if profiler_step_summary:
                ...
                print("⚠️ Error: Could not find the summary event 'ProfilerStep*'.")
                print("   Please ensure that prof_prefill.step() was called inside the profiler loop.")
    ...
    try: 
        ...
        with torch.profiler.profile( 
            ...
            # 1. use the key_averages() to get the aggregated data of 'ProfilerStep*' events
            key_averages_events = prof_decode.key_averages()
            ...
            if profiler_step_summary:
                ...
                print("⚠️ Error: Could not find the summary event 'ProfilerStep*'.")
                print("   Please ensure that prof_decode.step() was called inside the profiler loop.")

            # =========================================================================
            # ===== code block ends =====
            # =========================================================================

        print("Decoding performance analysis successful!") 
